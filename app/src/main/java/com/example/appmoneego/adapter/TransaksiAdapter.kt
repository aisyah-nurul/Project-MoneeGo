...
class TransaksiAdapter(
    ...
    private fun rebuildDisplayList() {
        ...
        if (transfer.isNotEmpty()) {
            ...
            if (!collapsedHeaders.contains("TRANSFER")) transfer.forEach { newList.add(TransaksiItem.Item(it)) }
        }
        if (pemasukan.isNotEmpty()) {
            newList.add(TransaksiItem.Header("Pemasukan", "PEMASUKAN"))
            if (!collapsedHeaders.contains("PEMASUKAN")) pemasukan.forEach { newList.add(TransaksiItem.Item(it)) }
        }
        if (pengeluaran.isNotEmpty()) {
            newList.add(TransaksiItem.Header("Pengeluaran", "PENGELUARAN"))
            if (!collapsedHeaders.contains("PENGELUARAN")) pengeluaran.forEach { newList.add(TransaksiItem.Item(it)) }
        }
    ...
    inner class HeaderViewHolder(private val binding: ItemHeaderTransaksiBinding) :
        ...
        fun bind(header: TransaksiItem.Header) {
            ...
            binding.root.setOnClickListener {
                if (isCollapsed) collapsedHeaders.remove(header.jenis) else collapsedHeaders.add(header.jenis)
                rebuildDisplayList()
            }
        }
    }
    ...
    inner class ItemViewHolder(private val binding: ItemTransaksiBinding) :
        ...
        fun bind(transaksi: Transaksi) {
            ...
            binding.tvSumberDana.text = daftarDompet.find { it.id == transaksi.dompetId }?.nama ?: "-"

            // LOGIKA MELIPAT (Collapse)
            binding.clHeaderInteraktif.setOnClickListener {
                isDetailExpanded = !isDetailExpanded
                val rotation = if (isDetailExpanded) 0f else -180f
                binding.ivArrowCollapse.animate().rotation(rotation).setDuration(200).start()

                binding.cardDetail.visibility = if (isDetailExpanded) View.VISIBLE else View.GONE
            }

            // Klik Card Utama buat Edit/Hapus
            binding.clCardUtama.setOnClickListener {
                ...
                android.app.AlertDialog.Builder(context)
                    ...
                    .setItems(pilihan) { _, which ->
                        if (which == 0) onEditClick(transaksi) else onDeleteClick(transaksi)
                    }.show()
            }
        }
    }

    override fun getItemViewType(position: Int) = if (displayList[position] is TransaksiItem.Header) TYPE_HEADER else TYPE_ITEM
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val inflater = LayoutInflater.from(parent.context)
        return if (viewType == TYPE_HEADER) HeaderViewHolder(ItemHeaderTransaksiBinding.inflate(inflater, parent, false))
        else ItemViewHolder(ItemTransaksiBinding.inflate(inflater, parent, false))
    }
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        val item = displayList[position]
        if (item is TransaksiItem.Header) (holder as HeaderViewHolder).bind(item)
        else if (item is TransaksiItem.Item) (holder as ItemViewHolder).bind(item.transaksi)
    }
